class Player {
  ...
  boolean killboss(Boss Bosses) {
    ...
    if (Bosses.loc.x - Bosses.sz/2 < loc.x + szx && Bosses.loc.x + Bosses.sz/2 > loc.x && Bosses.loc.y + Bosses.sz/2 > loc.y && Bosses.loc.y - Bosses.sz/2 < loc.y+szy){
//    abs(swordCenter.x - bossCenter.x) < (szx+Bosses.sz)/2 &&  abs(swordCenter.y - bossCenter.y) < (szy+Bosses.sz)/2) {
      println("boss dies");
      return true;
    } else {
      return false;
